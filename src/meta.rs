...
pub fn get_metalist_from_attr(attr: &syn::Attribute) -> Option<MetaSplittedByCommaToken>
{
	let metalist = attr.meta.require_list().ok()?;
	metalist
		.parse_args_with(|buf: &syn::parse::ParseBuffer| buf.parse_terminated(|a| a.parse::<syn::Meta>(), Token![,]))
		.ok()
}
...
pub fn get_value_in_meta_namevalue(metalist: &MetaSplittedByCommaToken, name: impl AsRef<str>) -> Option<&syn::Expr>
{
	metalist.iter().find_map(|meta| {
		let namevalue = meta.require_name_value().ok()?;
		namevalue.path.is_ident(name.as_ref()).then_some(&namevalue.value)
	})
}
...
///
/// -> #[attr( name = "value", name2 = "value2" )]
pub fn get_value_lit_in_meta_namevalue(metalist: &MetaSplittedByCommaToken, name: impl AsRef<str>)
	-> Option<&syn::Lit>
{
	metalist.iter().find_map(|meta| {
		let namevalue = meta.require_name_value().ok()?;
...
/// -> #[attr( prop, prop2 )]
#[inline]
pub fn has_path_in_meta_path(metalist: &MetaSplittedByCommaToken, prop: impl AsRef<str>) -> bool
{
